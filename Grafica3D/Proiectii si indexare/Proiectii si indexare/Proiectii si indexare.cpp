...
const glm::mat4 GetProjectionMatrix(bool bOrthoProj)
{
    ...
    if (bOrthoProj) {
        float fScaleFactor = 2000.f;
        projMatrix = glm::ortho<float>(-width / fScaleFactor, width / fScaleFactor,
            -height / fScaleFactor, height / fScaleFactor,
            -zFar, zFar);
    }
    else {
        float aspectRatio = width / height;
        projMatrix = glm::perspective<float>(glm::radians(FOV), aspectRatio, zNear, zFar);
...
const GLchar* VertexShader =
{
    ...
    "void main()\n"\
    "{\n"\
    "  gl_Position = WorldMatrix * in_Position * ProjMatrix;\n"\
    "  ex_Color = in_Color;\n"\
    "}\n"
};
...
void RenderFunction()
{
    ...
    glm::mat4 worldTransf = glm::translate(glm::mat4(1.0), glm::vec3(0, 0, -1));
    worldTransf = glm::scale(worldTransf, glm::vec3(0.1));
    worldTransf = glm::rotate(worldTransf, triangleOffset, glm::vec3(0, 1, 0));
    glm::mat4 projTransf = GetProjectionMatrix(g_OrthoProj);
